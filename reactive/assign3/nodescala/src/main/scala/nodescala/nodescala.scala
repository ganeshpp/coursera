...
trait NodeScala {
  ...
  val listner_sub=listener.start()
  val can= Future.run()(ct=>
             {
     async{
       while(ct.nonCancelled) {
       val f= listener.nextRequest()
       val(req,exch)=Await.result(f,3 second)
       respond(exch,ct,handler(req))

     }
   }})

   Subscription(listner_sub,cts)
  }
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      val promise = Promise[(Request,Exchange)]()
            ...
            def handler(exch: Exchange)= {
              promise.success((exch.request,exch))
              removeContext()
            }
