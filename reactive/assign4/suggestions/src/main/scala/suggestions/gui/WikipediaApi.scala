  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] ={
      Observable {
        ...
        observer: Observer[Try[T]] =>  {

      obs.materialize.subscribe( n => n match {
      case OnNext(v)  => { observer.onNext(Success(v))  }
      case _:OnCompleted[_] => observer.onCompleted()
      case OnError(s) => observer.onNext(Failure(s));observer.onCompleted()
    ...
    def timedOut(totalSec: Long): Observable[T] = {
     val obj= Observable.interval(totalSec seconds)
      obs.takeUntil(obj)
    }
