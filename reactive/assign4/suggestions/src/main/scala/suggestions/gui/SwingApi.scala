...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable { observer =>
      val rea = new scala.swing.Reactions.Reaction {
        def apply(v1: scala.swing.event.Event): Unit = {
        observer.onNext(field.text)};
        def isDefinedAt(x: scala.swing.event.Event):Boolean = {true}
      }
      field.subscribe(rea)
      Subscription{}
    }
